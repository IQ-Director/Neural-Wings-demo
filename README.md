...
### 4. 🎨 帧合成编辑器 (Frame Composition Editor)
为了简化复杂的后处理设计，项目内置了**可视化帧合成编辑器**：
*   **节点化工作流**：支持以节点拖动的方式制作渲染管线。
*   **可视化配置**：支持添加 **Pass 节点、粒子节点、贴图节点**，并直观连接各节点的 `inputs` 与 `outputs`。
*   **一键生成**：实时预览后处理逻辑，并可直接导出为引擎可读的 `view/renderview.json` 配置文件。
