...
{
    // When disabled, clear all states and return early.
    if (!m_enabled)
    {
        for (auto &pair : m_actionStates)
        {
            pair.second.isPressed = false;
            pair.second.isReleased = pair.second.isDown; // generate release if was down
