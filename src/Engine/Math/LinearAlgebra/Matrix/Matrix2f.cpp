...
#include "Engine/Math/LinearAlgebra/Vector/Vector2f.h"
...
{
	memcpy( m_data, rm.m_data, 2 * sizeof( float ) );
}
...
{
	float determinant = m_data[ 0 ] * m_data[ 3 ] - m_data[ 2 ] * m_data[ 1 ];
	...
	else
	{
		...
		return Matrix2f
		(
			m_data[ 3 ] * reciprocalDeterminant, -m_data[ 2 ] * reciprocalDeterminant,
			-m_data[ 1 ] * reciprocalDeterminant, m_data[ 0 ] * reciprocalDeterminant
		);
	}
}
